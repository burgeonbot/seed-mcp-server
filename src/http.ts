...
import type { TradingBotAgent, TradingBotRun } from "./types.js";

const currentUserAgentsFrameName = "CurrentUserAgentsFrame";
const currentUserAgentRunsFrameName = "CurrentUserAgentRunsFrame";

function createTradingBotMcpServer(accessToken: string | undefined): McpServer {
  ...
  server.tool(
    ...
    "List Trading Bot agents visible to the authenticated ForgeIQ user.",
    {
  ...
  server.tool(
    ...
    "Get one Trading Bot agent visible to the authenticated ForgeIQ user. Code is omitted unless includeCode is true.",
    {
  ...
  server.tool(
    ...
    "Get the currently saved JavaScript code for one Trading Bot agent visible to the authenticated ForgeIQ user.",
    {
      agentId: z.string().min(1)
    },
    async ({ agentId }) => {
  ...
  server.tool(
    ...
    "Get the current/latest run document for one Trading Bot agent visible to the authenticated ForgeIQ user.",
    {
      agentId: z.string().min(1)
    },
    async ({ agentId }) => {
  ...
  server.tool(
    ...
    "Get logs for a Trading Bot run visible to the authenticated ForgeIQ user. Use tailLines/maxChars to keep analysis scalable.",
    {
  ...
  server.tool(
    ...
    "Get the current/latest run logs for a Trading Bot agent visible to the authenticated ForgeIQ user.",
    {
      agentId: z.string().min(1),
      tailLines: z.number().int().min(1).max(5000).optional().default(500),
      maxChars: z.number().int().min(1000).max(500000).optional().default(120000)
  ...
  server.tool(
    ...
    "Update a Trading Bot agent's saved code. Use only after the user confirms the exact target agent and code change.",
    {
      agentId: z.string().min(1),
      code: z.string().min(1)
    },
  ...
  server.tool(
    ...
    "Start or deploy a Trading Bot agent. Use only after the user confirms the target agent and mode.",
    {
      agentId: z.string().min(1),
  ...
  server.tool(
    ...
    "Stop a running Trading Bot run. Use only after the user confirms the run id.",
    {
      runId: z.string().min(1)
    },
    async ({ runId }) => {
...
function requireAccessToken(accessToken: string | undefined): void {
  if (!accessToken) {
...
async function getVisibleAgent(client: SeedClient, agentId: string): Promise<TradingBotAgent> {
  const page = await client.listFrameDocuments<TradingBotAgent>(
    currentUserAgentsFrameName,
    0,
    1,
...
async function getVisibleRun(client: SeedClient, runId: string): Promise<TradingBotRun> {
